...
PROVIDERS = {
    ...
    "openrouter": {
        ...
        "edit_url": "https://openrouter.ai/api/v1/chat/completions",
    ...
    "google": {
        ...
        "edit_url": "https://generativelanguage.googleapis.com/v1beta/models/imagen-4.0-generate-001:predict",
        "key_env": "GOOGLE_API_KEY",
        "key_sops": "GOOGLE_API_KEY",
        "model": "imagen-4.0-generate-001",
        "use_google": True,
    },
    "huggingface": {
        "url": "https://api-inference.huggingface.co/models/black-forest-labs/FLUX.1-schnell",
        "edit_url": "https://api-inference.huggingface.co/models/black-forest-labs/FLUX.1-schnell",
        "key_env": "HF_API_KEY",
        "key_sops": "HF_API_KEY",
    ...
    "digitalocean": {
        ...
        "edit_url": "https://inference.do-ai.run/v1/images/edits",
        "key_env": "DIGITALOCEAN_API_KEY",
        "key_sops": "DIGITALOCEAN_API_KEY",
        "model": "openai-gpt-image-2",
    },
    "gemini": {
        # Gemini 2.5 Flash Image — free tier, generateContent endpoint
        "url": "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image:generateContent",
        "edit_url": "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image:generateContent",
        "key_env": "GOOGLE_API_KEY",
        "key_sops": "GOOGLE_API_KEY",
    ...
    "pollinations": {
        ...
        "edit_url": "https://image.pollinations.ai/prompt/",
        "key_env": "",
        "key_sops": "",
        "model": "flux",
    ...
    "together": {
        # Together AI — FLUX.1-schnell, ~$0.003/image, $1 free on signup
        # Sign up: https://api.together.ai → API Keys → Create
        "url": "https://api.together.xyz/v1/images/generations",
        "edit_url": "https://api.together.xyz/v1/images/generations",
        "key_env": "TOGETHER_API_KEY",
        "key_sops": "TOGETHER_API_KEY",
        "model": "black-forest-labs/FLUX.1-schnell",
        "use_together": True,
    },
    "fal": {
        # Fal.ai — FLUX.1-schnell, $5 free credits on signup
        # Sign up: https://fal.ai → Dashboard → API Keys
        "url": "https://fal.run/fal-ai/flux/schnell",
        "edit_url": "https://fal.run/fal-ai/flux/schnell",
        "key_env": "FAL_API_KEY",
        "key_sops": "FAL_API_KEY",
...
def api_request(
    ...
    prov = PROVIDERS[provider]
    model_name = prov.get("model", MODEL)
    ...
    if is_edit:
    ...
    elif prov.get("use_google"):
        # Google Imagen API — key as query param, returns base64 in predictions
        url = prov["url"] + f"?key={api_key}"
        # Map size to aspect ratio
        aspect = "9:16"
        if size == "1024x1024":
            aspect = "1:1"
        elif size in ("1792x1024",):
            aspect = "16:9"
        body = {
        ...
        for attempt in range(4):
            try:
                ...
                sys.exit(1)
        sys.exit(1)
    elif prov.get("use_hf"):
        # Hugging Face Inference API — returns raw binary image bytes
        url = prov["url"]
        body = {
            "inputs": prompt,
            "parameters": {
        ...
        for attempt in range(4):
            try:
                with urllib.request.urlopen(req, timeout=300) as resp:
                    raw = resp.read()
                    # HF returns raw PNG bytes — convert to base64
                ...
                if e.code == 503:
                    wait = 20
                    print(f"Model loading, retrying in {wait}s (attempt {attempt+1}/4)...", file=sys.stderr)
                    time.sleep(wait)
                    continue
                print(f"Error {e.code}: {body_err}", file=sys.stderr)
                sys.exit(1)
        print("Error: HF model failed to load after 4 attempts.", file=sys.stderr)
        sys.exit(1)
    elif prov.get("use_gemini_image"):
        # Gemini generateContent with IMAGE responseModality — free tier
        import urllib.parse
        url = prov["url"] + f"?key={api_key}"
        body = {
        ...
        for attempt in range(4):
            try:
                with urllib.request.urlopen(req, timeout=300) as resp:
                    ...
                    if not images:
                        print(f"Error: No image in Gemini response:\n{json.dumps(result, indent=2)}", file=sys.stderr)
                        sys.exit(1)
                    return images
            except urllib.error.HTTPError as e:
                err_body = e.read().decode("utf-8", errors="replace")
                ...
                sys.exit(1)
        sys.exit(1)
    elif prov.get("use_pollinations"):
        # Pollinations.ai — completely free, no auth, GET request returns PNG bytes
        import urllib.parse
        ...
        if size == "1024x1792":
            ...
            w, h = 1792, 1024
        seed = int(time.time())
        url = f"{prov['url']}{encoded}?width={w}&height={h}&model=flux&nologo=true&seed={seed}"
        req = urllib.request.Request(url, method="GET")
        req.add_header("User-Agent", "BrahhmContentStudio/3.1")
        for attempt in range(4):
            try:
                with urllib.request.urlopen(req, timeout=300) as resp:
                    raw = resp.read()
                ...
                sys.exit(1)
        sys.exit(1)
    elif prov.get("use_together"):
        # Together AI — OpenAI-compatible images endpoint with width/height params
        url = prov["url"]
        # Together uses integer width/height not "size" string
        w, h = 1024, 1024
        if size == "1024x1792": w, h = 1024, 1792
        elif size == "1792x1024": w, h = 1792, 1024
        body = {
            "model": model_name,
            "prompt": prompt,
            "n": n,
    ...
    elif prov.get("use_fal"):
        # Fal.ai — FLUX endpoints, returns image URLs
        url = prov["url"]
        # Map size to fal image_size presets
        fal_size = "portrait_9_16"
        if size == "1024x1024": fal_size = "square_hd"
        ...
        for attempt in range(4):
            try:
                with urllib.request.urlopen(req, timeout=300) as resp:
                    ...
                    if not images:
                        print(f"Error: No image in fal response:\n{json.dumps(result, indent=2)}", file=sys.stderr)
                        sys.exit(1)
                    return images
            except urllib.error.HTTPError as e:
                err_body = e.read().decode("utf-8", errors="replace")
                ...
                sys.exit(1)
        sys.exit(1)
    elif prov.get("use_chat_completions"):
        ...
        headers = {
            "Authorization": f"Bearer {api_key}",
            "Content-Type": "application/json",
            "HTTP-Referer": "https://brahhm-content-studio",
            "X-Title": "Brahhm Content Studio",
        }
    else:
        url = prov["url"]
        body: dict[str, Any] = {
    ...
    for attempt in range(max_retries):
        try:
            with urllib.request.urlopen(req, timeout=300) as resp:
                ...
                # OpenRouter chat completions image response
                if prov.get("use_chat_completions"):
                    images = []
                                ...
                                if isinstance(part, dict):
                                    ...
                                    if part.get("type") == "image_url":
                                        ...
                                        if url_data.startswith("data:"):
                                            images.append(url_data.split(",")[1])
                                        else:
                                            images.append(url_data)
                                    # b64_json format
                    ...
                    if not images:
                        print(f"Error: No image in response. Full response:\n{json.dumps(result, indent=2)}", file=sys.stderr)
                        sys.exit(1)
                    return images
...
def cmd_generate(args):
    ...
    needs_no_key = PROVIDERS[provider].get("use_pollinations") or not PROVIDERS[provider].get("key_env")
    api_key = ("no-key-needed" if needs_no_key else get_api_key(provider))
    if not api_key and not needs_no_key:
